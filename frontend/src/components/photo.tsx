...
import { recordKey } from '../lib/recordTimeline';
...
export function PatientMeasurementPhotoSection({ records }: { records: VitalRecord[] }) {
  ...
  // Carrossel (um registro por vez) em `SlideCarousel` — mesmo componente do
  // carrossel de indicadores do registro.
  return (
    <section className="bg-card border border-border rounded-xl shadow-sm p-5 md:p-6">
      <SlideCarousel
        items={relevant}
        getKey={(r) => recordKey(r)}
        renderItem={(r) => <RecordPhotosBlock record={r} />}
        header={() => (
          <>
        ...
        dotLabel={(i) => `Ir para a foto ${i + 1}`}
        empty={<p className="text-sm text-muted-foreground">Nenhuma foto enviada neste período.</p>}
      />
    </section>
  );
}
