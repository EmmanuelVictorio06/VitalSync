      ...
      <div className="sm:hidden">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-sm font-extrabold tracking-tight text-foreground truncate">{STEP_LABELS[current]}</p>
          </div>
...
function Connector({ done }: { done: boolean }) {
  return <span className={cn('h-px w-6 sm:w-8 transition-colors', done ? 'bg-stable' : 'bg-border')} aria-hidden="true" />;
}
